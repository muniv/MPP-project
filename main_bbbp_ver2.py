...
def train(model, device, loader, optimizer, evaluator):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            pred = model(batch)
            # 두 가지 다른 증강된 데이터 생성
            augmented_batch1 = augment_data(batch, device)
            augmented_batch2 = augment_data(batch, device)

            # 모델을 사용하여 각 증강에 대한 예측을 계산
            pred1 = model(augmented_batch1)
            pred2 = model(augmented_batch2)
